...
//    pos = _pos;
    ...
    pos.y = ofRandom(ofGetHeight());
     pos.z = ofRandom(ofGetHeight());
    mass= ofRandom(1.,3.);
    randir= ofVec3f (ofRandom(ofGetWidth()),ofRandom(ofGetHeight()),ofRandom(ofGetHeight()));
...
//  direction=direction.getNormalized() *180;
    ...
    randir.x=star.x+sin(ofGetElapsedTimef()/2*ofMap(mass,1,3,0.5,5))*(ofMap(mass,1.,3.,20.,300.));
    randir.y=star.y+cos(ofGetElapsedTimef()/2* ofMap(mass,1,3,0.5,5))*(ofMap(mass,1.,3.,20.,300.));
    randir.z=star.z+ofGetHeight()/2*sin(ofMap(mass,1.,3.,-1,1));
...
//
    float hue = ofMap(mass, 1, 3, 100, 150);
     float bright = ofMap(mass, 1, 3, 150, 250);
